...
from ..SharedCode.consts import CONN_STRING, LOGS_STARTS_WITH, FINDING_DETAILS_QUERY, CHECKPOINT_DATA_QUERY
...
class CheckpointManager:
    ...
    def remove_truncated_data(self, data):
        ...
        try:
            ...
            applogger.error("{} REMOVE TRUNCATED DATA: JSONDecodeError: Unable to loads JSON data from Sentinel table. Error: {}".format(LOGS_STARTS_WITH, err))
            raise BitSightException()
        except Exception as err:
            applogger.error("{} REMOVE TRUNCATED DATA: Exception: Unknown error occurred while loads JSON data from Sentinel table. Error: {}".format(LOGS_STARTS_WITH, err))
            raise BitSightException()
    ...
    def set_checkpoint_file_content(self, state: StateManager, content, company_flag):
        ...
        try:
            for data in content:
                ...
                if "[" in val_val and "]" in val_val:
                    try:
                        ...
                        applogger.error("{} SET CHECKPOINT FILE CONTENT: JSONDecodeError: Unable to loads JSON data for {} key from Sentinel table.".format(LOGS_STARTS_WITH, key_val))
                        continue
                elif "[" in val_val:
                    applogger.info("{} SET CHECKPOINT FILE CONTENT: Truncated data found for {} key from Sentinel table.".format(LOGS_STARTS_WITH, key_val))
            ...
            applogger.info("{} SET CHECKPOINT FILE CONTENT: Checkpoint file data updated from corrupted data.".format(LOGS_STARTS_WITH))
            if company_flag:
                return val_val
            return checkpoint_content
        except Exception as error:
            applogger.error(
                "{} SET CHECKPOINT FILE CONTENT Error: {}".format(LOGS_STARTS_WITH, error)
            )
            raise BitSightException()
    ...
    def get_last_data(
        ...
        try:
            ...
            applogger.error(
                "{} GET LAST DATA: JSONDecodeError: {}".format(LOGS_STARTS_WITH, error)
            )
            if company_name_flag:
            ...
            applogger.debug(
                "{} Fetching Checkpoint data from table: {}".format(LOGS_STARTS_WITH, table)
            )
            logs_data, logs_flag = get_logs_data(
                checkpoint_query.format(table)
            )
            if logs_flag:
    ...
    def save_checkpoint(
        ...
        try:
            if company_name_flag:
                ...
                applogger.info(
                    "{} save_checkpoint: {} -> {} successfully posted data.".format(
                        LOGS_STARTS_WITH, endpoint, data
                    )
                )
            else:
                if data is None:
    ...
    def save_checkpoint_to_sentinel(self, checkpoint_key, checkpoint_value, table_name):
        ...
        try:
            ...
            applogger.error(
                "{} SAVE CHECKPOINT TO SENTINEL: {}".format(LOGS_STARTS_WITH, err)
            )
            raise BitSightException()
    ...
    def convert_data_to_dict(self, key, value):
        ...
        try:
            ...
            applogger.exception(
                "{} Error Occurred while creating dict from key value: {}".format(
                    LOGS_STARTS_WITH, err
                )
            )
            raise BitSightException()
