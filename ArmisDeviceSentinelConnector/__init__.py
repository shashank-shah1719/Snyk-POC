...
class ArmisDevice:
    ...
    def compare_time_stamp(self, last_seen_time):
        last_seen_time = self.validate_timestamp(last_seen_time)
        last_seen_time = datetime.datetime.strptime(last_seen_time, "%Y-%m-%dT%H:%M:%S")
        current_time_utc = datetime.datetime.utcnow()
        time_difference_hours = (
            current_time_utc - last_seen_time
        ).total_seconds() / 3600
        if time_difference_hours < TIME_DELAY:
            return True
        return False

    def ingest_remaining_data_from_chunk_in_sentinel(self, data, sentinel_obj, checkpoint_obj, table_name):
        number_of_objects = len(data)
        for i in range(number_of_objects-1, -1, -1):
            last_seen_time = data[i].get("lastSeen")[:19]
            if self.compare_time_stamp(last_seen_time):
                number_of_objects = number_of_objects - 1
            else:
                last_seen_time_of_last_object = last_seen_time
                break
        new_data = data[:number_of_objects]
        sentinel_obj.post_data(customer_id, json.dumps(new_data), table_name)
        logging.info(
            "{}: Collected: {} device data and ingested into sentinel with table name: {}.".format(
                self.logs_starts_with, len(new_data), table_name
            )
        )
        checkpoint_obj.merge(
            "armisdevice",
            "devicecheckpoint",
            {"last_seen": last_seen_time_of_last_object}
        )
        logging.info(
            "{}: Updated last_seen to '{}' in Checkpoint table".format(
                self.logs_starts_with, last_seen_time_of_last_object
            )
        )
        return last_seen_time_of_last_object
    ...
    def _fetch_device_data(
        ...
        try:
            ...
            while self._data_device_from is not None:
                ...
                time_difference_hours = (
                    current_time_utc - last_seen_time
                ).total_seconds() / (3600 * TIME_DELAY)         # need to check this logic whether needs to use multiplication or not
                if time_difference_hours < TIME_DELAY:
                    self.last_seen = self.ingest_remaining_data_from_chunk_in_sentinel(data, azuresentinel, checkpoint_table_object, table_name)
                    logging.info(
                        "{}: Ingested data which are older than delay time: {} hr into Sentinel."
                        "lastSeen of last record: {}".format(
                            self.logs_starts_with, TIME_DELAY, self.last_seen
                        )
                    )
                    break
                ...
                logging.info("{}: last_seen time is: {}".format(self.logs_starts_with, last_seen_time))
                last_seen_time = last_seen_time.strftime("%Y-%m-%dT%H:%M:%S")
                self.last_seen = last_seen_time
                logging.info("{}: self.last_seen time is: {}".format(self.logs_starts_with, last_seen_time))
                checkpoint_table_object.merge(
                    "armisdevice",
                    "devicecheckpoint",
                    {"last_seen": self.last_seen}
                )
                logging.info("{}: After Checkpoint Update last_seen time is: {}".format(self.logs_starts_with, last_seen_time))
                logging.info(
                    "{}: Updated last_seen to '{}' in Checkpoint table".format(
                        self.logs_starts_with, last_seen_time
                    )
